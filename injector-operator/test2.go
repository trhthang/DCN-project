...
func main() {
	...
	for _, clusterWrapper := range lc.Spec.Clusters {
		...
		for _, packageWrapper := range cluster.Spec.Packages {
			pv := packageWrapper.PackageVariant
			fmt.Println("Package Name:", pv.Name)
			fmt.Println("Package Repo:", pv.Repo)
			fmt.Println("Package Package:", pv.Package)
			fmt.Println("Package Revision:", pv.Revision)
		}
		fmt.Println("------------------")
	}
}
